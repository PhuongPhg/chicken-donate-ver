...
export function getFactoryContract (){
  return new ethers.Contract(contractAddress as string, factoryJson.abi, signer)
}
...
export async function getOrganizationContract  (organizationContract: string) {
  const organizationContractAddress = await getOrganizationContractAddress(organizationContract);
  return new ethers.Contract(organizationContractAddress as string, organizationJson.abi, signer)
}
...
export async function createOrganization(name: string) {
  try {
    ...
    const organizationContractAddress = await getOrganizationContractAddress(signerAddress)
...
export async function getDonations(address: string){
  try {
    ...
    return (res || []).map((o: IDonationsEthers) =>( {amount: utils.formatEther(o?.amount), donorId: o.donorId, time: Number(o.time) }))
  } catch (error) {
